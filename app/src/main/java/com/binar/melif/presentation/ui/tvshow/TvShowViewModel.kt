...
import com.binar.melif.data.network.model.TvShow
...
class TvShowViewModel(private val tvShowRepository: TvShowRepository): ViewModel() {
    ...
    fun getTvShow(){
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val latestData = tvShowRepository.getLatestTvList()
            ...
            tvShowItems.apply {
                latestData.payload?.let {
                    add(TvShowItem.TvShowHeaderItem(it.random()))
                }
                sectionPopularList.payload?.let {
                    add(TvShowItem.TvShowSectionItem(R.string.popular_tv_section,it))
                }
                latestData.payload?.let {
                    add(TvShowItem.TvShowSectionItem(R.string.latest_tv_section,it))
                }
                sectionTopRatedList.payload?.let {
