...
import com.binar.melif.auth.dialog.AuthDialog
import com.binar.melif.auth.dialog.OnMenuSelectedListener
...
class AuthActivity : BaseViewModelActivity<ActivityAuthBinding, AuthViewModel>(ActivityAuthBinding::inflate){
    ...
    private fun loginPopUp(){
        AuthDialog().apply {
            setOnMenuSelectedListener(object : OnMenuSelectedListener {
                override fun onLoginClicked(dialog: DialogFragment) {
                    signIn()
                    dismiss()
                }
            })
